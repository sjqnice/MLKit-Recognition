...
package com.sjqnice.mlkit.mlkit.barcodescanner;
...
public class WxGraphic extends GraphicOverlay.Graphic {
    ...
    public void setStillImageHeight(int height){
        this.stillImageHeight = height;
        postInvalidate();
    }
    ...
    public void draw(Canvas canvas) {
        ...
        circlePath.reset();
        // ▼在屏幕中间添加一个圆
        float translateY = stillImageHeight == 0? 0f : canvas.getHeight() - stillImageHeight - (canvas.getHeight() - stillImageHeight) / 2f;
        circlePath.addCircle(rect.left + (rect.right - rect.left) / 2f, rect.top + (rect.bottom - rect.top) / 2f + translateY, radius * mProgress, Path.Direction.CW);
        // ▼将剪裁边界设置为视图大小
        Region globalRegion = new Region((int) rect.left, (int) (rect.top + translateY), (int) rect.right, (int) (rect.bottom+ translateY));
        // ▼将 Path 添加到 Region 中
        ...
        valueAnimator.addUpdateListener(animation -> {
            ...
            if (animation.getCurrentPlayTime() >= animation.getDuration()) {
                if (handler != null && !isDestroy) {
                    handler.postDelayed(resetRunnable = this::startAnim, 1000);
                }
            }
            postInvalidate();
        });
